...
Members never log in. `/` is the in-product gym home for **Complexe sportif Claude-Robillard**.
...
3. Friendly URLs: `/m/complexe-claude-robillard/lat-pulldown`.
...
Keyed by gym slug (`complexe-claude-robillard`). No member account.

| Key | Contents |
|-----|----------|
| `complexe-claude-robillard-workout:v1:<slug>` | In-progress session JSON |
| `complexe-claude-robillard-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
