...
Seed gym: `complexe-claude-robillard`, `primaryColor: #00853F`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `complexe-claude-robillard` (fallback: first gym).
...
| Workout + history | localStorage keys `complexe-claude-robillard-workout:v1:<slug>` and `complexe-claude-robillard-workout-history:v1:<slug>` |
